...
/*   Updated: 2023/09/07 15:30:10 by zhamdouc         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
Channel::Channel(std::string name, std::string key, User *user) : _name(name), _key(key), _maxUsers(0) ,  _nbUser(1)
{
...
void	Channel::joinChannel(std::string key, User *user)
{
	...
	if (this->_maxUsers != 0 && this->_nbUser >= this->_maxUsers)
		return ;//mettre le message d'erreur
